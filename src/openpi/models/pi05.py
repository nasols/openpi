...
from openpi.models.utils.quat_utils import quat_multiply, get_deltas, quatvel_to_angvel, quaternion_to_rpy, quat_orientation_error, euler_rpy_to_quat, quat_orientation_error

from ril_franka.real_world.panda_util import panda_fg_jax, damped_jacobian_inverse_4dof_jax, quat_xyzw_to_R_jax
...
logging.basicConfig(
    filename='debug_jax.log',
    level=logging.INFO,
    format='%(asctime)s - %(message)s'
)

# En enkel Python-funksjon som skriver til loggen
def log_to_file(label, value):
    logger.log(level=103, msg=f"{label}:\n{value}")


# REMOVE @njit(cache=True)
def get_tf_mat_dh(a, d, alpha, theta):
...
def get_jacobian(joint_angles):
    ...
    dh = jnp.array([
        ...
        [0.0,     0.0,   -jnp.pi / 2.0,   joint_angles[1]],
        [0.0,     0.316,  jnp.pi / 2.0,   joint_angles[2]],
        [0.0825,  0.0,    jnp.pi / 2.0,   joint_angles[3]],
        [-0.0825, 0.384, -jnp.pi / 2.0,   joint_angles[4]],
        [0.0,     0.0,    jnp.pi / 2.0,   joint_angles[5]],
        [0.088,   0.0,    jnp.pi / 2.0,   joint_angles[6]],
        [0.0,     0.107,  0.0,            0.0],             # flange
        [0.0,     0.0,    0.0,           -jnp.pi / 4.0],    # yaw
        [0.0,     0.150,  0.0,            0.0],             # TCP
    ])

    # 2. Calculate T_EE (Full forward kinematics)
    T_EE = jnp.eye(4)
    # Using a simple loop is fine for JIT, but for many steps use jax.lax.scan
    for i in range(10):
        T_EE = T_EE @ get_tf_mat_dh(*dh[i])

    # 3. Calculate Jacobian columns
    # We collect columns in a list and stack at the end to avoid mutation
    columns = []
    T = jnp.eye(4)
    for i in range(7):
        T = T @ get_tf_mat_dh(*dh[i])
        
        p_diff = T_EE[:3, 3] - T[:3, 3]
        z_axis = T[:3, 2]
        
        # Linear velocity part: z_i cross (p_EE - p_i)
        v = jnp.cross(z_axis, p_diff)
        # Angular velocity part: z_i
        w = z_axis
        
        columns.append(jnp.concatenate([v, w]))
...
class Pi05(_model.BaseModel):
    ...
    def __init__(self, config: pi05_config.Pi05Config, rngs: nnx.Rngs):
        ...
        # Un-normalizer
        self.un_normalizer = _transforms.Unnormalize(self.norm_stats, use_quantiles=True)
        self.normalizer = _transforms.Normalize(self.norm_stats, use_quantiles=True)
    ...
    def embed_prefix(
        ...
        if obs.tokenized_prompt is not None:
            tokenized_prompt = jnp.asarray(obs.tokenized_prompt)
    ...
    def compute_loss_ki(
        ...
        tokens_up_to_FAST = jnp.asarray(observation.tokenized_prompt * ~observation.action_token_mask)
        FAST_tokens = jnp.asarray(observation.tokenized_prompt * observation.action_token_mask)
    ...
    def _compute_loss_ki(self, rng, observation: _model.Observation, train=False): 

        ki_loss = self.compute_loss_ki(rng, observation, train=train)
        observation = _model.Observation(
                images=observation.images,
                image_masks=observation.image_masks,
                state=observation.state,
                tokenized_prompt=observation.tokenized_prompt * ~observation.action_token_mask,
                tokenized_prompt_mask=~observation.action_token_mask,
                token_ar_mask=None,
                token_loss_mask=None,
            )
        return ki_loss, observation
    
    def _compute_loss_guided_inference(self, rng, observation: _model.Observation, actions, train=False): 
        batch_shape = actions.shape[:-2]
        preprocess_rng, noise_rng, time_rng, delay_rng = jax.random.split(rng, 4)

        # Guided inference prep
        ...
        if self.config.guided_inference: 
            v_t = self._compute_flow_insulated(
                prefix_tokens, prefix_mask, prefix_ar_mask,
                suffix_tokens, suffix_mask, suffix_ar_mask,
                adarms_cond
            )
        else: 
            v_t = self._compute_flow(
                prefix_tokens, prefix_mask, prefix_ar_mask,
                suffix_tokens, suffix_mask, suffix_ar_mask,
                adarms_cond
            )
        ...
        loss = jnp.sum(loss*postfix_action_mask, axis=-1) / (jnp.sum(postfix_action_mask, axis=-1) + 1e-8)

        return loss       

    def _compute_flow_insulated(
            self, 
            ...
            adarms_cond : at.Float[at.Array, "b *s emb"]
            ) : 
        
        # Prepping VLM pass - prefix
        prefix_attn_mask = make_attn_mask(prefix_mask, prefix_ar_mask)
        prefix_position = jnp.cumsum(prefix_mask, axis=1) - 1
        # Prepping Action expert pass - suffix
        ...
        suffix_attn_mask = make_attn_mask(full_input_mask, full_ar_mask)
        suffix_attn_mask = suffix_attn_mask[:, -suffix_tokens.shape[1]:, :]
        ...
        # Detach KV -- No loss from action expert to VLM 
        kv_cache_detached = jax.tree.map(jax.lax.stop_gradient, kv_cache)
        ### The kv-cache from the forward pass will have shapes according to the prefix tokens. 
        ### I.e. the kv-cache had shapes (1, 15, 1, 8, 256) and (1, 983, 1, 256) for q then k. 
        ### When running the action expert, we need masks and positions to match the kv-cache shapes. 
    ...
    def compute_loss(
        ...
        if self.ki_mode:
            ...
            ki_loss, observation = self._compute_loss_ki(rng, observation, train=train)
        ...
        if self.config.guided_inference: 
            ...
            loss = self._compute_loss_guided_inference(rng, observation, actions, train=train)
    ...
    def guided_inference(
        ...
        # A_prev = jnp.pad(A_prev, ((0,0), (0,pad_width), (0, self.action_dim - A_prev.shape[-1]))) # Right-pad A_prev with zeros to be H-length 
        A_prev = jnp.pad(A_prev, ((0,0), (0,pad_width), (0, self.action_dim - A_prev.shape[2]))) # Right-pad A_prev with zeros to be H-length 
        #####################
        ...
        def step(carry, _):
            ...
            # # X = jnp.concatenate([Xlin, Xq_ang], axis=-1) 
            # Z = Xq[:, 2]
            # Jz = jnp.array([get_jacobian(q)[2, :] for q in Q])
            # grad_c = -policy_dt * W_c[:, None]*(Z[:, None] - Z_d[:, None])*Jz 
            # e_c = jnp.cumsum(grad_c[::-1], axis=0)[::-1]
            # e_c = e_c[None, :, :]
        ...
        hist = {
            ...
            "x1_hat_hist": x1_hat_hist,  # clean estimate used for RTC guidance
            "x_next_hist": x_next_hist,  # latent after update
            "v_hist": v_hist,
            "g_hist": g_hist,
            "t_hist": t_hist,
            "A_prev": A_prev,
    ...
    def guided_inference_2(
        ...
        *,
        ...
        def velocity_and_clean_estimate(
            ...
            prefix_to_suffix_attn_mask = einops.repeat(
                prefix_mask,
                "b p -> b s p",
            ...
            full_attn_mask = jnp.concatenate(
                [prefix_to_suffix_attn_mask, suffix_attn_mask],
                axis=-1,
            )
